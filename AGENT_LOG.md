...
### [P] 按仓库现状重写详细设计说明书 v2（暗色强调）
- **时间**：11:21
- **Prompt 摘要**：grilling 收口后生成 `docs/详细设计文档v2.docx`：暗色封面/页眉/表头 + 正文浅底深字；不覆盖 v1；第 3 章按三页面共 8 个功能点；字段/路径/状态码以 API-CONTRACT v1 为准。
- **产出文件**：`docs/详细设计文档v2.docx`（新建，36010 bytes，mtime 2026-08-17 11:18）。未改 `package.json`。未覆盖 `docs/TripMate-Lite-详细设计说明书.docx`（v1 仍在，68707 bytes，mtime 2026-08-14 14:59）。临时脚本 `_gen-design-doc-v2.cjs` 仍在仓库根目录（计划要求生成后删除，尚未清理）。
- **关键决策**：
  - 与 PRD 冲突时文首声明以契约为准；登录/OA/K8s/权限写「无」；不插任何图
  - 第 3 章：3.1 行程列表 / 3.2 行程详情 / 3.3 AI 助手，共 8 个功能点；逻辑说明表固定「筛选项：」「显示字段：」，后端逻辑与接口路径一一对应
  - 封面用整页深色底表格模拟满版（`#0F172A`），不设文档级黑底，避免转 PDF 失真；标题自带编号，禁止 Word 自动多级编号
- **自检结果**：v2 Word 已落盘且 v1 未被覆盖；docx 依赖装在临时目录，未写入项目 `package.json`。本机未找到 LibreOffice `soffice`，PDF 页面抽查未完成（`docs/_preview_v2` 为空）。根目录临时脚本未删。
- **交接条件**：交付文件为 `docs/详细设计文档v2.docx`；原说明书保留。评审可人工打开 Word 抽查封面暗色、目录标题 `1`/`2.1`/`2.1.1`、功能点逻辑说明表与第 5 章时间表。遗留：删除 `_gen-design-doc-v2.cjs`；有 LibreOffice 时可再转 PDF 抽查。
